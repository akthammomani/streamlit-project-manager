...
def _try_register_dejavu():
    """
    Try to register DejaVu Sans so the '↳' arrow renders in PDF.
    Safe no-op if font is not available.
    """
    try_paths = [
...
def full_screen_login():
    ...
    with col:
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            submitted = st.form_submit_button("Sign in / Continue", use_container_width=False)
        if submitted:
            if not email:
...
def full_screen_project_gate(user_email: str):
    ...
    with col:
        ...
        st.markdown(
            "<h2 style='text-align:center;margin-top:8px;'>Choose or Create a Project</h2>",
            unsafe_allow_html=True,
        )
        ...
        with st.form("center_new_project", clear_on_submit=True):
            ...
            submit_new = st.form_submit_button("Create project", use_container_width=False, type="primary")
...
def _sort_by_date_then_name(items):
    def keyf(x):
        sd = x.get("start_date")
        return (sd is None, sd or pd.Timestamp.max.date(), str(x.get("name") or ""))
    return sorted(items, key=keyf)


def build_gantt_figure(pid: int, show_subtasks: bool):
    # Fetch and sort tasks by start date (None -> last), then name
    raw_tasks = _sort_by_date_then_name([_to_task_dict(t) for t in db.get_tasks_for_project(pid)])

    # Build rows. Use a unique Y key and a display label.
    rows = []
    key_seq = 0

    # Count rows so we can compute dynamic height
    for t in raw_tasks:
        start_fixed, end_fixed = _safe_dates_for_timeline(t["start_date"], t["end_date"])
        if start_fixed and end_fixed:
            key_seq += 1
            rows.append({
                "Key": f"T{t['id']}_{key_seq}",
                "Display": f"<b>{t['name']}</b>" if t["name"] else "<b>Untitled Task</b>",
        ...
        if show_subtasks:
            subs = _sort_by_date_then_name([_to_subtask_dict(s) for s in db.get_subtasks_for_task(t["id"])])
            for s in subs:
                st_fixed, en_fixed = _safe_dates_for_timeline(s["start_date"], s["end_date"])
                if st_fixed and en_fixed:
                    key_seq += 1
                    rows.append({
                        "Key": f"S{s['id']}_{key_seq}",
                        "Display": f"↳ {s['name']}" if s["name"] else "↳ (Untitled subtask)",
                        "Label": (s["name"] or "(Untitled subtask)"),
                        "Start": st_fixed,
                        "Finish": en_fixed,
    ...
    # Sort for axis order = chronological, tasks first, then subtasks
    df["LevelOrder"] = df["Level"].map({"task": 0, "subtask": 1})
    df_axis = df.sort_values(["Start", "LevelOrder", "Label"], ascending=[True, True, True])
    tickvals = df_axis["Key"].tolist()
    ticktext = df_axis["Display"].tolist()

    # Build figure using unique 'Key' as y; then override tick labels
    fig = px.timeline(
        df, x_start="Start", x_end="Finish", y="Key",
    ...
    # Dynamic height (approx 24 px per row + top/bottom for axes)
    nrows = len(tickvals)
    fig_height = max(360, int(24 * nrows + 140))
    ...
    # Light vlines at task boundaries only
    vline_dates = set()
    for _, tr in df[df["Level"] == "task"].iterrows():
        vline_dates.add(pd.to_datetime(tr["Start"]))
        vline_dates.add(pd.to_datetime(tr["Finish"]))
    for d in sorted(vline_dates):
        fig.add_vline(x=d, line_dash="dot", line_color="rgba(0,0,0,0.25)", line_width=1)
...
def render_collapsible_gantt(pid: int):
    ...
    res = build_gantt_figure(pid, show_subtasks)
    if not res:
...
with tab1:
    ...
    edited_tasks = st.data_editor(
        ...
        key=f"tasks_editor_{current_project.id}",
    )

    if CAN_WRITE and st.button("💾 Save task changes", key=f"save_tasks_btn_{current_project.id}"):
        try:
    ...
    if not all_tasks_for_picker:
        ...
        task_for_sub = st.selectbox(
            ...
            key=f"task_picker_for_subtasks_{current_project.id}",
        )
        ...
        if picked_task_id:
            ...
            if CAN_WRITE and st.button("💾 Save subtask changes", key=f"save_subs_btn_{picked_task_id}"):
                try:
...
with tab2:
    ...
    with c1:
        st.metric("Days Total", total_days)
    with c2:
        st.metric("Days Remaining", remaining_days)
    with c3:
        st.metric("Items (Open/Total)", f"{open_items}/{total_items}")
    with c4:
        st.metric("Overdue", overdue_items)
    with c5:
        st.metric("Overall % Complete", f"{overall_progress}%")

    st.markdown("---")

    st.markdown("### Timeline - Gantt Chart")
    gantt = build_gantt_figure(current_project.id, show_subtasks=True)
    if gantt:
        fig_g, fig_h = gantt
        st.plotly_chart(fig_g, width="stretch", config={"displaylogo": False})
    else:
        st.info("Add start/end dates to tasks to see them on the timeline.")
    st.markdown("---")
    ...
    with col1:
        ...
        status_counts = (
            ...
            if not dfA.empty
            else pd.DataFrame({"status": status_order, "count": [0, 0, 0]})
        )
        fig_status = px.bar(
            status_counts,
            y="status",
    ...
    with col2:
        ...
        assignee_counts = (
            ...
            .reset_index(name="count")
            if not dfA.empty
            else pd.DataFrame({"assignee": [], "count": []})
        )
    ...
    if not dfA.empty:
        soon_mask = (
            ...
            & (~dfA["is_done"])
            & (dfA["end_date"] >= today)
            & (dfA["end_date"] <= (today + pd.Timedelta(days=14)))
        )
        upcoming = dfA.loc[
            soon_mask, ["name", "_type", "assignee_email", "status", "end_date", "progress"]
    ...
    if not upcoming.empty:
        st.data_editor(
            ...
            key=f"upcoming_preview_{current_project.id}",
        )
    else:
    ...
    missing_dates = (
        dfA.loc[~dfA["has_dates"], ["name", "_type", "assignee_email", "status", "progress"]]
        if not dfA.empty
        else pd.DataFrame(columns=["name", "_type", "assignee_email", "status", "progress"])
    )
    overdue_df = (
        dfA.loc[dfA["is_overdue"], ["name", "_type", "assignee_email", "status", "end_date", "progress"]].sort_values(
            "end_date"
        )
        if not dfA.empty
        else pd.DataFrame(columns=["name", "_type", "assignee_email", "status", "end_date", "progress"])
    )
    if missing_dates.empty and overdue_df.empty:
        ...
        if not missing_dates.empty:
            ...
            st.data_editor(
                ...
                disabled=True,
                column_config={"Progress%": st.column_config.NumberColumn("Progress %", min_value=0, max_value=100, step=1, format="%d%%")},
                key=f"missing_preview_{current_project.id}",
            )
        if not overdue_df.empty:
            ...
            st.data_editor(
                ...
                disabled=True,
                column_config={"Progress%": st.column_config.NumberColumn("Progress %", min_value=0, max_value=100, step=1, format="%d%%")},
                key=f"overdue_preview_{current_project.id}",
            )
...
with tab3:
    ...
    if not _members:
        ...
        st.data_editor(
            ...
            disabled=True,
            width="stretch",
            column_config={"Email": st.column_config.TextColumn("Email"), "Role": st.column_config.TextColumn("Role")},
            key=f"members_preview_{current_project.id}",
        )
    ...
    if not CAN_WRITE:
        ...
        members_line = st.text_area("Add members by email (comma-separated)", key=f"add_members_line_{current_project.id}")
        role_choice = st.selectbox("Role for new members", ["viewer", "editor"], key=f"add_members_role_{current_project.id}")
        if st.button("Add Members", key=f"add_members_btn_{current_project.id}"):
...
def _kpi_from_df(dfA, p_start, p_end):
    ...
    return {
        ...
        "overall_pct": overall_progress,
    }


def build_project_pdf(project, include_subtasks=True, include_charts=True):
    """Return PDF bytes."""
    # Register font for arrow (if available)
    font_name = _try_register_dejavu()

    tasks, subtasks_map, members = _collect_all_for_pdf(project)
    # Chronological order for tasks + (within) subtasks; None dates last
    tasks_sorted = _sort_by_date_then_name(tasks)
    for t in tasks_sorted:
        subtasks_map[t["id"]] = _sort_by_date_then_name(subtasks_map.get(t["id"], []))

    dfA = _df_for_analytics(tasks_sorted, include_subtasks=include_subtasks)
    ...
    if font_name:
        for sname in ("Body", "H1", "H2", "Muted"):
            styles[sname].fontName = font_name
    ...
    # Tasks table (NO Description column)
    ...
    task_table = Table(task_rows, repeatRows=1, colWidths=[220, 60, 70, 70, 70, 160, 65])
    task_styles = [
    ...
    # Bold entire row for tasks; subtasks normal
    for ridx in range(1, len(task_rows)):
        row = task_rows[ridx]
        is_task = (row[1] == "Task")
        task_styles.append(("FONTNAME", (0, ridx), (-1, ridx), "Helvetica-Bold" if is_task else "Helvetica"))

    task_table.setStyle(TableStyle(task_styles))
    ...
    if include_charts and _HAS_KALEIDO:
        def _add_fig_with_title(title: str, fig):
            try:
                img_bytes = pio.to_image(fig, format="png", scale=2)
                item = KeepTogether([Paragraph(title, styles["Body"]), RLImage(io.BytesIO(img_bytes), width=700, height=320)])
                story.append(item)
                story.append(Spacer(1, 8))
            except Exception as e:
                story.append(
                    Paragraph(
                        f"Chart '{title}' could not be embedded (Kaleido error). "
                        f"If running on a fresh server, Chrome/Chromium may be missing. "
                        f"Try: <b>pip install 'plotly[kaleido]'</b> then run <b>plotly_get_chrome</b>. Error: {e}",
                        styles["Muted"],
                    )
                )
                story.append(Spacer(1, 6))
        ...
        g1 = build_gantt_figure(project.id, True)
        if g1 and g1[0]:
            _add_fig_with_title("Gantt Chart (with subtasks)", g1[0])
        g2 = build_gantt_figure(project.id, False)
        if g2 and g2[0]:
            _add_fig_with_title("Gantt Chart (without subtasks)", g2[0])
        ...
        _add_fig_with_title("Distribution by Status", fig1)

        # Assignee workload
        df_assg = (
            dfA.assign(assignee=dfA["assignee_email"].fillna("Unassigned")).groupby("assignee").size().reset_index(name="count")
        ...
        _add_fig_with_title("Workload by Assignee", fig2)

        story.append(PageBreak())
    else:
        story.append(Paragraph("Charts not embedded (kaleido not available). KPIs included above.", styles["Muted"]))
        story.append(Spacer(1, 8))
    ...
    upcoming = (
        dfA.loc[soon_mask, ["name", "_type", "assignee_email", "status", "end_date", "progress"]].sort_values("end_date")
        if not dfA.empty
        else pd.DataFrame()
    )
    ...
    if upcoming.empty:
        ...
        up_table = Table(up_rows, repeatRows=1, colWidths=[260, 70, 180, 90, 90, 80])
        up_table.setStyle(
            TableStyle(
                [
                    ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#f3f4f6")),
    ...
    if dfA.empty:
        ...
        dfA["has_dates"] = dfA["start_date"].notna() & dfA["end_date"].notna()
        missing_dates = dfA.loc[dfA["has_dates"].eq(False), ["name", "_type", "assignee_email", "status", "progress"]]
        overdue_df = dfA.loc[
            (dfA["end_date"].notna()) & (dfA["end_date"] < today) & (~dfA["status"].eq("Done")),
            ["name", "_type", "assignee_email", "status", "end_date", "progress"],
        ].sort_values("end_date")
        if missing_dates.empty and overdue_df.empty:
            ...
            if not missing_dates.empty:
                ...
                miss_table = Table(miss_rows, repeatRows=1, colWidths=[300, 70, 200, 90, 80])
                miss_table.setStyle(
                    TableStyle(
                        [
            ...
            if not overdue_df.empty:
                ...
                ov_table = Table(ov_rows, repeatRows=1, colWidths=[300, 70, 200, 90, 90, 80])
                ov_table.setStyle(
                    TableStyle(
                        [
    ...
    if not members:
        ...
        mem_table = Table(mem_rows, repeatRows=1, colWidths=[360, 140])
        mem_table.setStyle(
            TableStyle(
                [
                    ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#f3f4f6")),
...
with tab4:
    ...
    if st.button("📄 Generate PDF", key=f"gen_pdf_btn_{current_project.id}"):
        try:
            pdf_bytes = build_project_pdf(
                project=current_project, include_subtasks=include_sub, include_charts=include_charts
            )
            st.download_button(
                ...
                key=f"download_pdf_btn_{current_project.id}",
            )
        except Exception as e:
            st.error(
                "PDF export failed:\n\n"
                f"{e}\n\n"
                "If this is a Kaleido/Chrome issue, install a modern Kaleido build and Chrome:\n"
                "  pip install 'plotly[kaleido]'  # or: pip install 'kaleido>=1.0.0'\n"
                "  plotly_get_chrome"
            )
